...
with open(os.path.join(os.path.dirname(jupyterlab.__file__), 'themes', '@jupyterlab', 'theme-light-extension', 'index.css'), 'r') as fp:
    _JUPYTERLAB_STYLE = '<style>\n{}\n</style>'.format('''
/*-----------------------------------------------------------------------------
...
def html_to_png_imgkit(tablehtml):
    import imgkit
    doc = '<html><head>{}</head><body><div class="jp-RenderedHTMLCommon">{}</div>'.format(_JUPYTERLAB_STYLE, tablehtml)
    return image(imgkit.from_string(doc, False))
